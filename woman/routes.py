...
##宣告Blueprint route名稱##
woman = Blueprint('woman',__name__,url_prefix='/woman')
##
...
def home():
    return f'<h1>woman Recommend API</h1>'
...
@woman.route('/content_update')
def content_update():
    minute = datetime.now().minute
    table = cache.get('woman_update')
    if not table or (minute in [0,15,30,45]):
        content = get_data('woman_update', 1080)
        table = content.to_json(force_ascii=False)
        cache.set('woman_update',table,timeout=3600)
        return table
    else:
        return table

@woman.route('/article_cache', methods=['GET'])##後台文章API
...
#            now = datetime.now()
    ...
    health_table = cache.get('woman_cache_'+str(day))
    if day ==90:
...
#                if not news_table or(now_time >= time(00,0) and now_time <= time(00,2)) :
        ...
        if not health_table:
            ...
            health_table=jsonify(rows)
            health_table.status_code=200
            cache.set('woman_cache_'+str(day),health_table,timeout=3600)
            cur.close()
            conn.close()
            return health_table
        else:
...
@woman.route('/tag_score_table', methods=['GET'])##News 標籤推薦API
#    @cache.cached(timeout=5)
...
#    args = request.args
    ...
    tag_summary_list = cache.get('woman_tag_cache_tag_cache'+str(day)+search_console)
    try:
...
#            if (not news_tag_summary):
            print('Not cache')
            back_tag_of_dfItem = tra.cache_article_table('woman').get_aws_table_cache(day)
            if search_console =='Y':
                tag_summary = tra.editorTag('woman',back_tag_of_dfItem,'Y').editor_tag_summary()
            else:
                tag_summary = tra.editorTag('woman',back_tag_of_dfItem,'N').editor_tag_summary()
            summary_list = dataframe_to_json(tag_summary)
            tag_summary_list = jsonify(summary_list)
            tag_summary_list.status_code=200
            cache.set('woman_tag_cache_tag_cache'+str(day)+search_console,tag_summary_list,timeout=7200)
            return tag_summary_list
        else:
            print('woman_tag_cache_tag_cache')
            return tag_summary_list
    finally:
        print('request get /tvbs_woman_tag_analysis')

@woman.route('/google_scarch_console_tag',methods =['GET'])
def gsc_tag():
    hour = datetime.now().hour
    minute = datetime.now().minute
    gsc_table = cache.get('woman_tag_cache')
    if not gsc_table or \
        ((hour>0 and minute > 0) and (hour>0 and minute <= 1)) or ((hour>6 and minute > 0) and (hour>6 and minute <= 1)) or \
        ((hour>8 and minute > 0) and (hour>8 and minute <= 1)) or ((hour>10 and minute > 0) and (hour>10 and minute <= 1)) or \
        ((hour>12 and minute > 0) and (hour>12 and minute <= 1)) or ((hour>14 and minute > 0) and (hour>14 and minute <= 1)) or \
        ((hour>16 and minute > 0) and (hour>16 and minute <= 1)) or ((hour>18 and minute > 0) and (hour>18 and minute <= 1)) or \
        ((hour>20 and minute > 0) and (hour>20 and minute <= 1)) or ((hour>22 and minute > 0) and (hour>22 and minute <= 1)):
        tag_gsc = read_gzip.tmp_read('dict','woman')
        ...
        cache.set('woman_tag_cache',gsc_table,timeout=7200)
        return gsc_table
    else:
        print('woman_tag_cache')
        return gsc_table


@woman.route('/post_tag_recommend',methods=['POST'])##News 推薦文章API
def tvbs_tag_recommend():
    result={}
    temp_json  = request.get_json(force=True)
    tag_recommentTop20 = tagrec.get_tag_recommend('woman',temp_json['article'],'Y')
    result = {'recomment_tag':tag_recommentTop20}
    return jsonify(result)

@woman.route('/tags', methods=['GET'])
def tags():
#        now = datetime.now()
#        now_time = now.time()
    table = cache.get('tag')
...
@woman.route('/recommend_list', methods=['GET'])
def result():
    minute = datetime.now().minute
...
@woman.route('/post_recommend',methods=['POST'])##News 推薦文章API
